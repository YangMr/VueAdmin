<template>
  <div class="tags-view-container">
    <ul class="d-flex">
      <li :class="{active : item.path === $route.path}" @click="handleToTarget(item.path)" v-for="(item,index) in tags"
          :key="item">
        <span>{{ item.title }}</span>
        <i @click.stop="handleDeleteTag(index,item.path)" v-if="item.path === $route.path" class="close">x</i>
      </li>
    </ul>
  </div>
</template>
...
const handleDeleteTag = (index, routePath) => {
  ...
  if (index === tags.value.length - 1) {
    if (index !== 0) {
      path = tags.value[index - 1].path ? tags.value[index - 1].path : tags.value[index].path
    } else {
